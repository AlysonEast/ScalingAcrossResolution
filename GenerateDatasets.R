...
cat("\n--- Starting 1 ha crown processing for", site, product, "---\n")
...
#st_write(grid, "./Shapefiles/grid_100m.shp")
...
cat("Found", length(files), "shapefiles to process.\n\n")
...
for (i in seq_along(files)) {
  ...
  if (i %% 10 == 0) {
    checkpoint_file <- paste0("../ScalingAcrossResolution/CrownDatasets/",
                              site, "_", product, "_trees_partial.csv")
    write.csv(crowns_assigned_df, checkpoint_file, row.names = FALSE)
    cat("   -> Checkpoint saved after", i, "files.\n")
  }
...
#### Final save ####
out_file <- paste0("../ScalingAcrossResolution/CrownDatasets/", site, "_", product, "_trees.csv")
